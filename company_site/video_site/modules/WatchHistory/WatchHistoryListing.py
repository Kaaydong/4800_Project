...
from . import WatchHistoryFunctions as whf

class WatchHistoryListing:
    ...
    def __init__(self, age_restriction, user_id):
        ...
        self.watched_movies = whf.getWatchEntriesByUser(user_id)


    # Generates all of the data needed for a movie card
    def __generateWatchEntryCardInfo(self, movie):
        ...
        return [movie, restriction_formatted, timestamp_formatted, genres_formatted, isBookmarked]


    # get a list of all movies watched by a user, and filtered by age restriction
    def getWatchedMovies(self):
        returned_list = []
        for movie in self.watched_movies:
            if movie.watch_progress >= 1 and self.age_restriction >= movie.movie_key.age_restriction:
                returned_list.append(self.__generateWatchEntryCardInfo(movie.movie_key))
